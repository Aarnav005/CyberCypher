...
function MiniChart({ values, color }: any) {
  ...
  if (!values || values.length === 0) {
    return (
      <div className="chart" style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', color: 'var(--muted)', fontSize: 11, background: 'rgba(255,255,255,0.03)', borderRadius: 4 }}>
        Gathering data...
      </div>
    )
  }
...
export default function TelemetryCard({ status }: any) {
  ...
  const successSeries = status?.success_series ?? []
  const latencySeries = status?.latency_series ?? []
  const total = status?.total_volume ?? 0
  const failRate = status?.fail_rate ?? 0
